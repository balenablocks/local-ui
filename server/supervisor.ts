...
class SupervisorProxy {
	...
	private async get(route: string) {
		return await this.request.get(route).then(({ data }) => {
			return data;
		});
	}

	private async post(route: string) {
		return await this.request.post(route).then(({ data }) => {
			return data;
		});
	}
	...
	private async queryAPIAndTransform(
		...
		const promises = fields.map(async (field) => {
			...
			try {
				...
				console.error(
					`[LocalUI] [SupervisorProxy] Error querying ${method.toUpperCase()} ${endpoint}: ${
						(error as NodeJS.ErrnoException).message || error
					}`,
				);
